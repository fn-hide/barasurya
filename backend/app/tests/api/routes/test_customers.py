...
def test_create_customer(client: TestClient, db: Session) -> None:
    ...
    data = {"name": "Foo", "phone": "+6281234567890", "address": "St. Groove", "customer_type_id": str(customer_type.id)}
...
def test_read_customers_superuser(client: TestClient, superuser_token_headers: dict[str, str], db: Session) -> None:
...
def test_update_customer(client: TestClient, db: Session) -> None:
    ...
    user_token_headers = authentication_token_from_email(client=client, email=user.email, db=db)
    customer = create_random_customer(db, user)
    data = {"name": "Updated name", "phone": "Updated phone", "address": "Updated address"}
    response = client.put(
        f"{settings.API_V1_STR}/customers/{customer.id}",
        headers=user_token_headers,
        json=data,
